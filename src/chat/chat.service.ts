...
작성 규칙:
- intro_message는 350~800자 정도의 자연스러운 한국어 답변
- 한 문장으로 줄이지 말고, 필요하면 짧은 문단과 줄바꿈을 사용
...
- 추천 메뉴나 피드백 메뉴가 있으면 단품/조합/다음 끼니 조절 같은 실행 가능한 팁을 포함
...
`.trim();
    ...
    try {
      ...
      return introMessage.slice(0, 1000);
    } catch {
      return params.fallback;
    }
  }
